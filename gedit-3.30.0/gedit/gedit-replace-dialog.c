...
//Function to list similar type of Files , written below call back function.
static void
on_list_similar_files_toggled(GtkToggleButton *toggle_button, gpointer user_data)
{
    GeditReplaceDialog *dlg = GEDIT_REPLACE_DIALOG(user_data);
	...
	//implementation.
    	char path[256], ext[10];
        
        g_print("Enter path and file extension:\n");
        scanf("%255s %9s", path, ext);
        ...
        // Concatenating extension
        strcat(command, ext);
        strcat(command, "'");
        
        // Execute the command
        g_print("All Files in above directory are as follow:");
	system(command);
...
on_list_similar_files_toggled(GtkToggleButton *toggle_button, gpointer user_data)
{
    ...
    if (gtk_toggle_button_get_active(toggle_button))
    {
        ...
        // Ask for path and file extension using a GTK dialog
	    ...
	    g_print("Executing command: %s\n", command); // Debugging output
            ...
            // Store file names
            char file_list[20][256];  // Max 20 files for simplicity
            int file_count = 0;
            while (fgets(file_list[file_count], sizeof(file_list[file_count]), fp) && file_count < 20)
            {
                file_list[file_count][strcspn(file_list[file_count], "\n")] = 0; // Remove newline
                file_count++;
            }
            pclose(fp);

            // Show the fetched files in another dialog
            GtkWidget *file_dialog, *file_area, *file_box, *check_buttons[file_count];

            file_dialog = gtk_dialog_new_with_buttons("Select Files",
                                                      NULL,
                                                      GTK_DIALOG_MODAL,
                                                      "_OK", GTK_RESPONSE_OK,
                                                      "_Cancel", GTK_RESPONSE_CANCEL,
            ...
            gtk_container_add(GTK_CONTAINER(file_area), file_box);

            // Add checkboxes for each file
            for (int i = 0; i < file_count; i++)
            {
                check_buttons[i] = gtk_check_button_new_with_label(file_list[i]);
                gtk_box_pack_start(GTK_BOX(file_box), check_buttons[i], FALSE, FALSE, 0);
            }
            ...
            {
                g_print("User selected files:\n");
                for (int i = 0; i < file_count; i++)
                {
                    if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(check_buttons[i])))
                    {
                        g_print("%s\n", file_list[i]); // Process selected files here
                    }
                }
            }
